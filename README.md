...
由此带来的变化是，这些前端应用可以**独立运行**、**独立开发**、**独立部署**。

就当前而言微前端架构，一般可以由以下的几个方式进行：

 1. 在不同的框架之上设计通讯、加载机制，诸如 [Mooa](https://github.com/phodal/mooa) 和 [Single-SPA](https://github.com/CanopyTax/single-spa)
 2. 使用 HTTP 服务器的路由来重定向不同应用，即不同的应用运行在不同的路由上。
 3. 使用纯 WebComponent 构建应用。
 4. 使用 WebComponent 和 ShadowDOM 技术来隔离不同的应用。
 5. 使用 iFrame 及 postMessage 进行通讯
